...
use crate::command::{Command, GotoMode, JumpMode, MoveAmount, MoveMode, TargetMode, SortKey, SortDirection};
    ...
    pub fn sort(&self, key: &SortKey, direction: &SortDirection) {
        fn compare_artists(a: Vec<String>, b: Vec<String>) -> Ordering {
            let sanitize_artists_name = |x:Vec<String>| -> Vec<String> { 
                x.iter().map(|x|x.to_lowercase().split(' ').skip_while(|x|x == &"the").collect()).collect()
            };
        ...
        c.sort_by(|a,b|match (a.track(), b.track()){
            (Some(a), Some(b)) => match (key, direction) {
                (SortKey::Title, SortDirection::Ascending) => a.title.to_lowercase().cmp(&b.title.to_lowercase()),
                (SortKey::Title, SortDirection::Descending) => b.title.to_lowercase().cmp(&a.title.to_lowercase()),
                ...
                (SortKey::Album, SortDirection::Ascending) => a.album.map(|x|x.to_lowercase()).cmp(&b.album.map(|x|x.to_lowercase())),
                (SortKey::Album, SortDirection::Descending) => b.album.map(|x|x.to_lowercase()).cmp(&a.album.map(|x|x.to_lowercase())),
                ...
                (SortKey::Artist, SortDirection::Ascending) => compare_artists(a.artists, b.artists),
                (SortKey::Artist, SortDirection::Descending) => compare_artists(b.artists, a.artists),
            }
